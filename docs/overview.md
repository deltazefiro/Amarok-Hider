...
- **State:** `Hider.state: StateFlow<State>` — derived from repository-backed app/folder state plus transient processing sets and a controller-level processing flag. The flag keeps `processAll()` in `PROCESSING` across its app-then-folder handoff so the dashboard does not flicker through settled states mid-run. `Hider.getState()` exists for synchronous service/entrypoint reads.
