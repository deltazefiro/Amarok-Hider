...
- **State:** `Hider.state: StateFlow<State>` — derived from repository-backed app/folder state plus transient per-item processing sets. Aggregate state stays `PROCESSING` while any app or folder entry is processing, so dashboard/quick-hide observers do not see a settled state until all active targets finish.
- **Unified process:** `Hider.processAll(context, Hider.Action)` computes all managed app/folder targets, marks them processing up front, then runs app and folder hider work concurrently; blocking I/O runs on `Dispatchers.IO`.
- **Targeted operations:** `Hider.processApps()` and `Hider.processFolders()` handle explicit app/folder sets. They ignore keys already processing, but no longer block different keys.
