...
public class ObfuscateFileHider extends BaseFileHider {
  ...
  private static final int MAX_PROCESS_WHOLE_FILE_SIZE_KB = 10 * 1024; // In KB.
  private static final int MAX_PROCESS_ENHANCED_WHOLE_FILE_SIZE_KB = 30 * 1024;
   ...
   * @param extraEndingMark (only effective when `HIDE`) Extra mark to be append to the end of the
   *     filename.
  ...
  private Path processFilename(Path path, ProcessMethod method, String extraEndingMark) {
    ...
    if (method == HIDE) {
      ...
      newFilename =
          "." + Base64.encodeToString(filename.getBytes(UTF_8), BASE64_TAG) + extraEndingMark;
