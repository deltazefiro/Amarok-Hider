...
public class FileHiderUtil {
  ...
  private static final List<String> COMMON_TEXT_EXTENSION = List.of(".txt", ".md", ".lrc");
  private static final int ASSUME_IS_TEXT_FILE_SIZE_THRESHOLD_KB = 5 * 1024;
  ...
  public static boolean checkIsTextFileEnhanced(Path path) {
    ...
    try {
      ...
      Log.w(
          "FileHider",
          String.format("%s: Failed to check is text: ", path.getFileName().toString()),
          e);
      return false;
    }
    ...
    if (type == null) {
      ...
      Log.d(
          "FileHider", String.format("%s: Failed to probe MIME: ", path.getFileName().toString()));
