...
public class XHidePrefBridge {
  ...
  private static final String LOCAL_PREF_DIR =
      String.format("/data/data/%s/shared_prefs", BuildConfig.APPLICATION_ID);
  ...
  /**
   * Enabling the module in Xposed framework will cause lose of original preference, due to the new
   * XSharedPreferences redirection. This function will copy the preferences in Amarok's local
  ...
  public static void migratePrefsIfNeeded(Context context) {
    ...
    Log.w(
        TAG,
        String.format(
            "Try to migrate ordinary preferences to XPref directory: " + "%s -> %s",
