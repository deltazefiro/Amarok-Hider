...
public class SwitchLocaleUtil {
  public static void switchLocale(Context context) {
    ...
    if (activeLocale.equals(LocaleListCompat.getEmptyLocaleList())) rgRadioGroup.check(0);
    else rgRadioGroup.check(Arrays.asList(LangList.LOCALES).indexOf(activeLocale.toLanguageTags()));
        ...
        (group, checkedId) -> {
          ...
          if (checkedId == 0) {
            ...
            LocaleListCompat appLocale =
                LocaleListCompat.forLanguageTags(LangList.LOCALES[checkedId]);
            AppCompatDelegate.setApplicationLocales(appLocale);
          }
        });

    // Show dialog
    new MaterialAlertDialogBuilder(context).setView(radioGroupView).show();
  }
  ...
  public static Locale getActiveLocale(Context context) {
    ...
    else return Locale.forLanguageTag(activeLocale.toLanguageTags());
  }
}
