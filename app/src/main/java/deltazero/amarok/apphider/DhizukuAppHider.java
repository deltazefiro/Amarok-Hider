...
public class DhizukuAppHider extends BaseAppHider {
  ...
  public void tryToActivate(ActivationCallbackListener activationCallbackListener) {
    ...
    if (!Dhizuku.init()) {
      ...
      activationCallbackListener.onActivateCallback(
          this.getClass(), false, R.string.dhizuku_not_init);
      return;
    }
    ...
    if (Dhizuku.getVersionCode() < 5) {
      ...
      activationCallbackListener.onActivateCallback(
          this.getClass(), false, R.string.dhizuku_pre_v5);
      return;
    }
    ...
    Dhizuku.requestPermission(
        new DhizukuRequestPermissionListener() {
          @Override
          public void onRequestPermission(int grantResult) {
            if (grantResult != PackageManager.PERMISSION_GRANTED) {
              Log.d("DhizukuHider", "Permission denied.");
              activationCallbackListener.onActivateCallback(
                  DhizukuAppHider.class, false, R.string.dhizuku_permission_denied);
              return;
            }
            ...
            try {
              ...
              activationCallbackListener.onActivateCallback(
                  DhizukuAppHider.class, false, R.string.dhizuku_failed_to_set_delegated_scopes);
              return;
            }
  ...
  private void setDelegatedScopes() {
    if (Arrays.asList(Dhizuku.getDelegatedScopes()).contains(DELEGATION_PACKAGE_ACCESS)) return;
