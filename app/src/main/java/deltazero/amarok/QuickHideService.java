...
public class QuickHideService extends LifecycleService {
  ...
  private void updatePanicButton() {
    if (!PrefMgr.getEnablePanicButton()) return;

    if (!XXPermissions.isGranted(getApplication(), Permission.SYSTEM_ALERT_WINDOW)) {
      Log.w(
          "QuickHideService", "Failed to show PanicButton: Permission denied: SYSTEM_ALERT_WINDOW");
    ...
    if (Hider.getState() == Hider.State.PROCESSING) {
      ivPanicButton.setColorFilter(
          getApplication().getColor(com.google.android.material.R.color.design_default_color_error),
      ...
      ivPanicButton.setColorFilter(
          PrefMgr.getPanicButtonColor(), android.graphics.PorterDuff.Mode.SRC_IN);
