...
public class QSTileService extends TileService {
  ...
  public void onClick() {
    unlockAndRun(
        () -> {
          ...
          switch (Hider.getState()) {
            case VISIBLE -> Hider.processAll(this, Hider.newHideAction());
            case HIDDEN -> {
              if (SecurityUtil.isUnlockRequired()) startAuthThenUnhide();
              else Hider.processAll(this, HideAction.Unhide.INSTANCE);
            }
            default -> throw new IllegalStateException("Unexpected value: " + Hider.getState());
          }
        });
  ...
  /**
   * The method should be invoked in {@link AmarokApplication#onCreate()}, after {@link Hider#state}
   * is initialized.
   *
  ...
  private void startAuthThenUnhide() {
    var intent =
        new Intent(this, SecurityAuthForQSActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
