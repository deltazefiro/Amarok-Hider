...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val managedFolderCount: StateFlow<Int> = MutableStateFlow(PrefMgr.getHideFilePath().size).asStateFlow()

    private val _appHiderName = MutableStateFlow(
        BaseAppHider.fromMode(application, PrefMgr.getAppHiderMode()).name
    )
    val appHiderName: StateFlow<String> = _appHiderName.asStateFlow()

    private val _fileHiderName = MutableStateFlow(
        BaseFileHider.fromMode(application, PrefMgr.getFileHiderMode()).name
    )
